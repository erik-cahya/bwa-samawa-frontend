          ...
          return (
            <div key={slide.id} className="card-slide h-full rounded-3xl overflow-hidden relative">
              ...
              <div className="card-slide-content flex flex-col items-start gap-y-5">
                ...
                <Link href={`${process.env.HOST_APP}/packages/${slide.slug}`} className="flex justify-center bg-color2 py-2 w-full text-light1 rounded-full">
                  View Package
                </Link>
              </div>
            </div>
      ...
      {data.map((weddingPackage) => {
        return (
          <div className="flex flex-col gap-y-4 relative">
            <Link href="/wedding-details.html" className="absolute inset-0 z-10"></Link>
            <span className="relative h-[300px] rounded-3xl overflow-hidden">
              <span className="absolute z-10 top-5 left-5">
                <span className="bg-color1 rounded-full text-light1 inline-flex gap-x-2 items-center text-sm py-1 px-3 uppercase">
                  <Popular />
                  {`${weddingPackage.isPopular === 1 ? "Popular" : "Not Popular"}`}
                </span>
              </span>
              <Image
                fill
            ...
            <span className="flex flex-col gap-[14px]">
              ...
              {/* <span className="flex gap-x-2 items-center">
                        <svg className="text-color1" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                           <path
                              opacity="0.4"
